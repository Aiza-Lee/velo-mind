...
public:
	...
	size_t size_bytes() const { return numel() * data_element_size(); }

	template <typename T> T* data() { return static_cast<T*>(_storage->data()); }
	template <typename T> const T* data() const { return static_cast<const T*>(_storage->data()); }
