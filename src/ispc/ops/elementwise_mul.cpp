...
extern "C" void mul_f32_ispc(float* a, float* b, float* c, int n);

template <typename T1, typename T2, typename T3>
void mul_ispc_impl(const TensorStorage* const* in,
                   TensorStorage* const*      out,
                   const void*                 ) {
    static_assert(std::is_same_v<T1, float> && std::is_same_v<T2, float> && std::is_same_v<T3, float>,
                  "ISPC Mul only supports float32");
    ...
    mul_f32_ispc(const_cast<float*>(a), const_cast<float*>(b), c, n);
}

VELOMIND_REGISTER_BINARY_KERNEL(DeviceType::ISPC, Op::Mul,
                                DataType::Float32, DataType::Float32,
                                DataType::Float32, mul_ispc_impl)

}

VELOMIND_REGISTER_SIGNATURE(Op::Mul, ::velomind::detail::signature_elementwise)
