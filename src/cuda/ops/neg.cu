...
template <typename T>
__global__ void neg_kernel(const T* x, T* y, std::size_t n) {
...
void neg_impl(const TensorStorage* const* in, TensorStorage* const* out, const void* ) {
    ...
    neg_kernel<<<blocks, threads, 0, stream>>>(x, y, n);
    check_cuda_kernel(Op::Neg, *out[0]);
}

namespace {
    static ::velomind::internal::KernelRegistrar _velomind_kr_neg_f32(
        DeviceType::CUDA, Op::Neg,
        ::velomind::internal::KernelDtypeKey{
            { DataType::Float32 }, 1, DataType::Float32 },
        static_cast<Executable::KernelFn>(&neg_impl<float>));
}
