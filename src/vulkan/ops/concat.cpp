...
void concat_impl(const TensorStorage* const* in,
    ...
    static_assert(std::is_same_v<T1, T2> && std::is_same_v<T1, T3>,
                  "Concat: in1/in2/out dtype must match");
    ...
    for (std::size_t i = 0; i < rank; ++i) {
        ...
        if (a_shape[i] != b_shape[i] || a_shape[i] != o_shape[i]) {
            throw std::runtime_error("velomind::Concat (Vulkan): non-axis dim mismatch");
        }
    }
    if (a_shape[axis] + b_shape[axis] != o_shape[axis]) {
        throw std::runtime_error("velomind::Concat (Vulkan): axis sum mismatch");
    }
