...
namespace {
    ...
    auto softmax_pipeline() -> SoftmaxPipeline& {
        static SoftmaxPipeline p;
        static std::once_flag once;
        std::call_once(once, [&]() {
            if (!backend_vulkan::ensure_vulkan_ready()) return;
            auto& state = backend_vulkan::vulkan_state();

            auto bytes = backend_vulkan::load_shader_spv("softmax");
    ...
    void softmax_impl(const TensorStorage* const* in, TensorStorage* const* out,
        ...
        if (!pipe.ok)
            throw std::runtime_error(vulkan_error_context(Op::Softmax, *out[0], "pipeline") +
                                     ": initialization failed");
        check_vulkan(VK_SUCCESS, Op::Softmax, *out[0], "pipeline");
        ...
        if (storage_numel(*in[0]) > std::numeric_limits<uint32_t>::max())
            throw std::invalid_argument("Softmax Vulkan: tensor exceeds index range");

        // v1 每次调用独立录制并同步，资源池在 fence 完成后立即复用。
        ...
        check_vulkan(desc_alloc_res, Op::Softmax, *out[0], "descriptor allocation");
        ...
        struct PC { uint32_t rows; uint32_t cols; uint32_t inner; } pc{
        ...
        check_vulkan(cmd_alloc_res, Op::Softmax, *out[0], "command allocation");
        ...
        check_vulkan(vkBeginCommandBuffer(cmd, &cbbi),
                     Op::Softmax, *out[0], "command begin");
