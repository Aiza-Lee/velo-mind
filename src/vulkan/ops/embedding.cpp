...
namespace {
    ...
    auto embedding_pipeline() -> EmbeddingPipeline& {
        static EmbeddingPipeline p;
        static std::once_flag once;
        std::call_once(once, [&]() {
            if (!backend_vulkan::ensure_vulkan_ready()) return;
            auto& state = backend_vulkan::vulkan_state();

            auto bytes = backend_vulkan::load_shader_spv("embedding");
    ...
    void embedding_impl(const TensorStorage* const* in, TensorStorage* const* out,
        ...
        if (!pipe.ok)
            throw std::runtime_error(vulkan_error_context(Op::Embedding, *out[0], "pipeline") +
                                     ": initialization failed");
        check_vulkan(VK_SUCCESS, Op::Embedding, *out[0], "pipeline");

        const std::size_t hidden    = static_cast<std::size_t>(in[0]->shape.back());
        const std::size_t num_idx   = storage_numel(*in[1]);
        const std::size_t total     = num_idx * hidden;
        if (total == 0) return;
        ...
        check_vulkan(desc_alloc_res, Op::Embedding, *out[0], "descriptor allocation");
        ...
        struct PC { uint32_t num_indices; uint32_t hidden; } pc{
            static_cast<uint32_t>(num_idx),
        ...
        check_vulkan(cmd_alloc_res, Op::Embedding, *out[0], "command allocation");
        ...
        check_vulkan(vkBeginCommandBuffer(cmd, &cbbi),
                     Op::Embedding, *out[0], "command begin");
