...
namespace {
    ...
    auto mul_pipeline() -> MulPipeline& {
        static MulPipeline p;
        static std::once_flag once;
        std::call_once(once, [&]() {
            if (!backend_vulkan::ensure_vulkan_ready()) return;
            auto& state = backend_vulkan::vulkan_state();

            auto bytes = backend_vulkan::load_shader_spv("elementwise_mul");
    ...
    void mul_impl(const TensorStorage* const* in, TensorStorage* const* out,
                  const void* ) {
        auto& state = backend_vulkan::vulkan_state();
        auto& pipe  = mul_pipeline();
        if (!pipe.ok)
            throw std::runtime_error(vulkan_error_context(Op::Mul, *out[0], "pipeline") +
                                     ": initialization failed");
        check_vulkan(VK_SUCCESS, Op::Mul, *out[0], "pipeline");
        ...
        check_vulkan(desc_alloc_res, Op::Mul, *out[0], "descriptor allocation");
        ...
        check_vulkan(cmd_alloc_res, Op::Mul, *out[0], "command allocation");
