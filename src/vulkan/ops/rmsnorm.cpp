...
namespace {
    ...
    auto rmsnorm_pipeline() -> RMSNormPipeline& {
        static RMSNormPipeline p;
        static std::once_flag once;
        std::call_once(once, [&]() {
            if (!backend_vulkan::ensure_vulkan_ready()) return;
            auto& state = backend_vulkan::vulkan_state();

            auto bytes = backend_vulkan::load_shader_spv("rmsnorm");
    ...
    void rmsnorm_impl(const TensorStorage* const* in, TensorStorage* const* out,
        ...
        if (!pipe.ok)
            throw std::runtime_error(vulkan_error_context(Op::RMSNorm, *out[0], "pipeline") +
                                     ": initialization failed");
        check_vulkan(VK_SUCCESS, Op::RMSNorm, *out[0], "pipeline");

        const auto& desc  = *static_cast<const OpDescriptor*>(attrs_ptr);
        const auto& rattr = std::get<RMSNormAttrs>(desc.attrs);

        const auto& shape = in[0]->shape;
        const int rank = static_cast<int>(shape.size());
        const int axis = rattr.axis < 0 ? rattr.axis + rank : rattr.axis;
        if (axis != rank - 1) return;

        const std::size_t cols    = static_cast<std::size_t>(shape[axis]);
        const std::size_t rows    = storage_numel(*in[0]) / cols;
        ...
        check_vulkan(desc_alloc_res, Op::RMSNorm, *out[0], "descriptor allocation");
        ...
        check_vulkan(cmd_alloc_res, Op::RMSNorm, *out[0], "command allocation");
        ...
        check_vulkan(vkBeginCommandBuffer(cmd, &cbbi),
                     Op::RMSNorm, *out[0], "command begin");
        ...
        vkCmdPushConstants(cmd, pipe.pipe_layout,
                           ...
                           0, sizeof(pc), &pc);
        vkCmdDispatch(cmd, static_cast<uint32_t>(rows), 1, 1);
