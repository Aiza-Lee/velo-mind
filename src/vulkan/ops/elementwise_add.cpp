...
namespace {
    ...
    auto add_pipeline() -> AddPipeline& {
        static AddPipeline p;
        static std::once_flag once;
        std::call_once(once, [&]() {
            if (!backend_vulkan::ensure_vulkan_ready()) return;
            auto& state = backend_vulkan::vulkan_state();

            auto bytes = backend_vulkan::load_shader_spv("elementwise_add");
    ...
    void add_impl(const TensorStorage* const* in, TensorStorage* const* out,
                  const void* ) {
        auto& state = backend_vulkan::vulkan_state();
        auto& pipe  = add_pipeline();
        if (!pipe.ok)
            throw std::runtime_error(vulkan_error_context(Op::Add, *out[0], "pipeline") +
                                     ": initialization failed");
        check_vulkan(VK_SUCCESS, Op::Add, *out[0], "pipeline");
        ...
        check_vulkan(desc_alloc_res, Op::Add, *out[0], "descriptor allocation");
        ...
        std::array<VkDescriptorBufferInfo, 3> buffer_infos = {
            ...
            VkDescriptorBufferInfo{ buffer_of(in[1]),  in[1]->offset_bytes, n * sizeof(float) },
        ...
        check_vulkan(cmd_alloc_res, Op::Add, *out[0], "command allocation");
        ...
        guard.finish(Op::Add, *out[0]);
    }

VELOMIND_REGISTER_BINARY_FN(DeviceType::VULKAN, Op::Add, DataType::Float32, DataType::Float32, DataType::Float32, &add_impl);
