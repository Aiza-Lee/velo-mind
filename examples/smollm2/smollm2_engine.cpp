...
auto make_bind_weights() -> LlamaEngine::BindWeightsFn {
    return [](velomind::Graph& g, const Config& cfg, std::size_t seq_len)
        -> ForwardWeights {
        return bind_smollm2_weights(g, cfg, seq_len);
    };
}

auto make_populate_weights() -> LlamaEngine::PopulateWeightsFn {
    return [](const velomind::SafetensorsFile& sf, const Config& cfg,
              ForwardWeights& w, std::size_t seq_len) -> void {
        populate_smollm2_weights(sf, cfg, w, seq_len);
    };
}
...
auto make_tokenizer_factory() -> LlamaEngine::TokenizerFactory {
    return []() -> std::unique_ptr<velomind::examples::llama::TokenizerAdapter> {
        return std::unique_ptr<velomind::examples::llama::TokenizerAdapter>(
            std::make_unique<HuggingFaceBpeTokenizer>());
    };
}
...
SmolLM2Engine::~SmolLM2Engine() = default;

void SmolLM2Engine::load() {
    engine_->load();
}

bool SmolLM2Engine::is_loaded() const noexcept {
    return engine_ != nullptr && engine_->is_loaded();
}

const velomind::examples::llama::TokenizerAdapter&
SmolLM2Engine::tokenizer() const noexcept {
    return engine_->tokenizer();
}

const EngineConfig& SmolLM2Engine::config() const noexcept {
    return cfg_;
}

auto SmolLM2Engine::prefill(std::span<const std::int32_t> tokens)
    -> std::vector<float> {
    return engine_->prefill(tokens);
}

auto SmolLM2Engine::forward_step(std::int32_t token, std::size_t pos)
    -> std::vector<float> {
    return engine_->forward_step(token, pos);
}

auto SmolLM2Engine::forward_logits(std::span<const std::int32_t> tokens)
    -> std::vector<float> {
    return engine_->forward_logits(tokens);
}

auto SmolLM2Engine::forward_full_logits(std::span<const std::int32_t> tokens)
    -> std::vector<float> {
    return engine_->forward_full_logits(tokens);
}

auto SmolLM2Engine::generate(
